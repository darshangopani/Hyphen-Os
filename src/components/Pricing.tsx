...
export function Pricing() {
  return (
    <section className="relative min-h-screen flex flex-col justify-center py-32 px-4 bg-deep-space/90 backdrop-blur-md border-t border-white/5 overflow-hidden">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="text-center mb-24">
          ...
          >
            ...
            <span className="text-xs font-mono tracking-widest text-slate-300 uppercase">System Access Levels</span>
          </motion.div>
          ...
          >
            Select Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-neon-blue via-neon-purple to-neon-blue animate-pulse">Protocol</span>
          </motion.h2>
          <div className="h-1 w-32 bg-gradient-to-r from-transparent via-neon-purple to-transparent mx-auto opacity-50" />
        </div>
            ...
            return (
              <motion.div
                ...
                className={`relative p-8 flex flex-col group ${
                  tier.highlight 
                    ? 'bg-black/60 border border-neon-purple/50 shadow-[0_0_40px_rgba(188,19,254,0.15)] lg:scale-105 z-10 lg:-mx-4 min-h-[600px]' 
                    : 'bg-black/40 border border-white/10 hover:border-white/30 min-h-[550px]'
                }`}
              >
                {/* Tech Corners */}
                <div className={`absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 ${tier.highlight ? 'border-neon-purple' : 'border-white/20'}`} />
                <div className={`absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 ${tier.highlight ? 'border-neon-purple' : 'border-white/20'}`} />
                <div className={`absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 ${tier.highlight ? 'border-neon-purple' : 'border-white/20'}`} />
                <div className={`absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 ${tier.highlight ? 'border-neon-purple' : 'border-white/20'}`} />

                {/* Scanning Line Effect for Highlighted Tier */}
                {tier.highlight && (
                  <div className="absolute inset-0 overflow-hidden pointer-events-none">
                    <div className="absolute top-0 left-0 w-full h-[2px] bg-neon-purple/50 shadow-[0_0_10px_rgba(188,19,254,0.8)] animate-scan" />
                  </div>
                )}

                {tier.highlight && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-6 py-1 bg-neon-purple text-white text-xs font-display font-bold tracking-widest rounded-sm shadow-[0_0_20px_rgba(188,19,254,0.4)]">
                    RECOMMENDED
                  </div>
                )}
                ...
                <div className="mb-8 relative z-10">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="text-[10px] font-mono text-slate-500 tracking-widest">{tier.sysCode}</div>
                  </div>
                  ...
                  <div className="text-slate-400 font-mono text-sm mb-6 h-10">{tier.target}</div>
                ...
                <ul className="space-y-5 mb-12 flex-grow font-mono text-sm text-slate-300 relative z-10">
                ...
                <button className={`w-full py-4 font-display font-bold tracking-widest uppercase transition-all duration-300 flex items-center justify-center gap-3 relative overflow-hidden group/btn z-10 ${
                  tier.highlight
                    ? 'bg-neon-purple text-white hover:shadow-[0_0_30px_rgba(188,19,254,0.6)]'
